...
## Candidate 1.6.0-rc.5: first real Claude conversation and Claude Desktop host parity

On 21 September 2026 the installed 1.6.0-rc.4 extension was exercised from an actual Claude conversation (Claude Code inside the Claude desktop app, sharing the extension server) side by side with the hosted RDC service on the same Mac. Text reads, writes, edits, moves, metadata, URL reads, DOCX/XLSX/PDF round trips in both directions, the PTY REPL and the Ego Lite browser workflow (example.com opened, `h1` extracted and verified in 2.9 s) all worked. Two defects were reproduced in that host: every pipe worker, batch and search failed as `PROCESS_LAUNCH_OUTCOME_UNCERTAIN` within 70 ms because Claude Desktop runs the extension as an Electron utility process whose `process.execPath` is the Claude binary (identical code passes in 163 ms under system Node), and `commander_list_sessions` took 7–19 s because every one of 1,683 session records was read for a `limit: 3` request. An XLSX write interrupted by an app quit was correctly recorded `uncertain` and quarantined its file, but the only release path was the `navish reconcile` CLI, so batches in that directory stayed `RESOURCE_QUARANTINED` from chat. Document calls in that host also paid several seconds of module loading on first use (ExcelJS 4.3 s versus 0.28 s under system Node).

The candidate resolves a plain Node.js executable before any launch claim, reads only the newest session page, warms the document engines after connect, adds `commander_ping` and `commander_reconcile`, enforces RDC's default `blockedCommands` and a configurable `defaultShell`, extends inline waits and multi-file reads, explains failed edits, aligns line counts with `wc -l`, and stops reporting a successful termination as an error. `test/claude-host-parity.test.mjs` reproduces the host condition with the real MCP server spawned from a hard-linked Node binary named `Claude` and passes; the full suite passes 149 tests. A control run of the fixed server under system Node measured pipe launch 277 ms, search 176 ms, two-worker batch 475 ms, first XLSX read 482 ms then 13 ms, `list_sessions` 2 ms. Verification inside the installed Claude Desktop extension, a fresh hosted RDC comparison, publication and the personal connector redeploy are recorded below this entry when they happen; until then this candidate is unpublished and claims no new certification.
...
## Connector 0.4.5: the agent survives its local server

The 0.4.4 agent opened its local Commander server once at module scope, with no retry and the MCP SDK's default 60-second timeout. Three exits are recorded in its log; each dropped the channel and whatever request was in flight. The worse case left no trace at all: a stdio child that died after startup kept the agent running against a server it could not reach, failing every later job with no crash for launchd to restart.

0.4.5 retries the connection with capped backoff, reconnects on demand, shares one connection attempt across concurrent jobs, and installs unhandled-rejection handlers ahead of all startup work so no exit is silent. [The resilience record](../evidence/connector-agent-resilience-20260922.json) covers the regression test, which hangs the previous build until timeout and passes the patched one, and the live check: killing the running Commander child left the agent up with launchd's run count unchanged, and the next job over the deployed ChatGPT path started a fresh server and completed. The suite passes 150 tests.

## Connector 0.4.4 / runtime 1.6.0-rc.5 installed and verified live

Runtime 1.6.0-rc.5 was published, installed into Claude Desktop from the downloaded release asset, and deployed across the personal ChatGPT connector on 21 September 2026. The [live verification record](../evidence/rc5-live-verification-20260921.json) measures each operation that failed on rc.4 in the same host: the pipe worker now completes in 167 ms instead of failing in 70 ms, search and three-worker batches run, the XLSX write that never returned now takes 90 ms, DOCX 81 ms and a two-page PDF 1.39 s, `commander_list_sessions` answers in 51 ms over 1,688 records, a failed edit reports its closest match, and a browser workflow no longer needs `expectedUrl` repeated. A direct protocol probe of the installed server confirms 35 tools, `commander_ping`, `commander_reconcile`, the raised inline waits and multi-file budget, and that `sudo` is refused before any launch.

That deployment also found and repaired a live outage: the Mac agent had been failing `CHANNEL_CONNECT_FAILED` since about 10:25 because the channel worker's `AGENT_TOKEN` no longer matched the agent configuration rotated at 00:18, so the ChatGPT path was already down before this work began. After restoring the pairing and redeploying, a complete owner OAuth/PKCE flow against the deployed HTTPS endpoint returned 36 tools and verified a ping, an exact write with independent readback, and a pipe worker completing through the connector.

No new hosted RDC throughput comparison was run for this candidate, so the recorded ratio remains the 0.4.1 result of 2.079x median with a 95% interval of 1.988-2.165x, which still fails the strict lower-bound gate. OpenAI's automatic safety review remains outside Commander's control.
