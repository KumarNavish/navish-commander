...
const recovery=JSON.parse(fs.readFileSync('evidence/chat-natural-recovery-20260921.json'));
assert.equal(recovery.schema,'navish.chat-natural-recovery/v1');
assert.deepEqual(recovery.source,record.source);
assert.equal(recovery.explicitPluginSelection,false);
assert.equal(recovery.outsideInterventionAfterSubmission,false);
assert.equal(recovery.mutations,0);assert.equal(recovery.platformDenials,0);
assert.deepEqual(recovery.calls.map(c=>c.name),['commander_devices','commander_read_file']);
const observed=recovery.calls[1];
assert.equal(observed.output.state,'completed');
assert.equal(observed.input.path,'$TRIAL/handoff.json');
assert.equal(observed.output.connectorOperation.toolName,observed.name);
assert.equal(observed.output.connectorOperation.path,observed.input.path);
assert.deepEqual(JSON.parse(observed.output.result.content),handoff);
assert.deepEqual(recovery.verifiedResult,handoff.results);
console.log(JSON.stringify({naturalLanguageRecovery:'passed',toolCalls:2,explicitPluginSelection:false}));
