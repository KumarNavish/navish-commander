...
const connectorFiles=['connector/handler.mjs','connector/auth.mjs','connector/queue.mjs','connector/journal.mjs','connector/poll-handler.mjs','connector/agent.mjs','connector/catalog.json','connector/functions/commander.mts','netlify/edge-functions/poll.ts','netlify.toml'];
const connectorDigest=()=>{const h=crypto.createHash('sha256');for(const f of connectorFiles)h.update(f).update(fs.readFileSync(f));return h.digest('hex');};
const connectorSourceSha256=connectorDigest();
const frozen={connectorSourceSha256,protocol,harnessSha256:sourceHash,commanderSourceSha256:sourceDigest(),gitRevision:revision()};
...
const sourceUnchanged=sourceDigest()===frozen.commanderSourceSha256&&connectorDigest()===connectorSourceSha256;
