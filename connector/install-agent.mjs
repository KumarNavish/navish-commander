...
// Chat requests need interactive latency. launchd's default classification
// throttles CPU/I/O and coalesces collection timers; XPC adaptive boosting is
// unavailable on this outbound WebSocket/stdio route. Keep idle network traffic
// bounded in the agent rather than using background scheduling for active work.
const plist=`<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd"><plist version="1.0"><dict><key>Label</key><string>${label}</string><key>ProgramArguments</key><array>${[process.execPath,path.join(release,'connector/agent.mjs'),configPath].map(v=>'<string>'+xml(v)+'</string>').join('')}</array><key>ProcessType</key><string>Interactive</string><key>LegacyTimers</key><true/><key>RunAtLoad</key><true/><key>KeepAlive</key><true/><key>ThrottleInterval</key><integer>30</integer><key>WorkingDirectory</key><string>${xml(release)}</string><key>EnvironmentVariables</key><dict><key>HOME</key><string>${xml(os.homedir())}</string><key>PATH</key><string>${xml(process.env.PATH)}</string></dict><key>StandardOutPath</key><string>${xml(path.join(config.stateDir,'stdout.log'))}</string><key>StandardErrorPath</key><string>${xml(path.join(config.stateDir,'stderr.log'))}</string></dict></plist>`;
